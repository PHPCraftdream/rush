...
func TestTracker_BuiltinSkillTracking(t *testing.T) {
	...
	// Simulate active skills including a builtin skill (rush-config).
	activeSkills := []*Skill{
		{Name: "rush-config", Description: "Rush config", Builtin: true},
		{Name: "go-doc", Description: "Go docs", Builtin: false},
	}
	tracker := NewTracker(activeSkills)

	// Initially not loaded.
	require.False(t, tracker.IsLoaded("rush-config"))
	require.False(t, tracker.IsLoaded("go-doc"))

	// Mark builtin skill as loaded (simulating read via rush://...).
	tracker.MarkLoaded("rush-config")
	require.True(t, tracker.IsLoaded("rush-config"))
