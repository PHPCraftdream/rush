...
the agent lifecycle, allowing you to both build on top of Rush, customize
...
our **project-level** `rush.json`. Relative paths like `./no-haskell.sh` work
here because the project root is your working directory. If you're configuring
a global hook (`~/.config/rush/rush.json`), use an absolute path instead.
...
# ($RUSH_TOOL_INPUT_COMMAND) to grep and match on a regexp.
if echo "$RUSH_TOOL_INPUT_COMMAND" | grep -qE '(^| )((ghc|cabal|stack)(\.exe)?)( |$)'; then
...
Hooks run through Rush's embedded POSIX shell (`mvdan.cc/sh`) — the same
...
- **Permissive shebang fallback**: if the absolute path in a shebang
  doesn't exist (e.g. `#!/bin/bash` on Windows), Rush falls back to a
...
- **Environment**: every hook sees `RUSH=1`, `AGENT=rush`, and
  ...
  both contexts.
- **Timeout behavior**: when a hook exceeds its timeout, Rush cancels the
  context and waits a short grace period (~1s) for the interpreter to
  yield. If the hook still hasn't returned, Rush abandons it, logs a
...
Hooks can be added to your `rush.json` (or `.rush.json`) at both the global
...
> work fine in project-level `rush.json` because the project root is also
> your working directory. For **global** config (`~/.config/rush/`),
> however, you must use either an absolute path or an inline command:
>
> ```jsonc
> // Global ~/.config/rush/rush.json
> {
>   "hooks": {
>     "PreToolUse": [
>       {
>         "command": "/home/you/.config/rush/hooks/no-haskell.sh"
...
3. Runs all matching hooks **in parallel** through Rush's embedded POSIX
...
5. Applies the result **before** permission checks. If the aggregated decision
   ...
   for it. If it's `allow`, Rush treats that as affirmative pre-approval and
...
however you'll incur some additional overhead as Rush will still parse and
run each hook.
...
Hooks communicate back to Rush via **exit code** and `stdout`/`stderr`. The
...
deterministic based on the order hooks appear in `rush.json`.
...
If a hook exceeds its timeout, Rush cancels its context and treats the
...
if echo "$RUSH_TOOL_INPUT_COMMAND" | grep -qE 'rm\s+-(rf|fr)\s+/'; then
...
returns `decision: "allow"`, which tells Rush to pre-approve the call:
...
case "$RUSH_TOOL_INPUT_COMMAND" in
...
if [[ "$RUSH_TOOL_INPUT_FILE_PATH" == *.go ]]; then
...
```jsonc
{ "command": "echo \"$(date -Iseconds) $RUSH_TOOL_NAME\" >> ./tools.log" }
```
...
Rush hooks are broadly compatible with [Claude Code
...
hooks run under Rush unchanged. This document covers the Rush-specific API
only — anything not documented here isn't guaranteed to work.

One intentional divergence: Rush treats `updated_input` as a shallow-merge
...
4. `decision` precedence: `deny` > `allow` > `null`. First deny determines the
   ...
   is `allow`, Rush pre-approves the tool call and skips the permission
