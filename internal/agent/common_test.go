...
func hyperBuilder(model string) builderFunc {
	return func(t *testing.T, r *vcr.Recorder) (fantasy.LanguageModel, error) {
		provider, err := openaicompat.New(
			...
			openaicompat.WithAPIKey(os.Getenv("RUSH_HYPER_API_KEY")),
...
func coderAgent(r *vcr.Recorder, env fakeEnv, smart, fast fantasy.LanguageModel) (SessionAgent, error) {
	...
	// independently of user config on `$HOME/.config/rush/rush.json`.
	...
	cfg.Config().Options.DisabledSkills = []string{"rush-config"}
