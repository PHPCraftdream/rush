...
func TestRecoverInterruptedTurns_NoLiveHolder_StillRecovers(t *testing.T) {
	...
	lk, err := session.TryAcquireSessionLockWithOptions(dataDir, sess.ID,
		session.WithCleanupDone(cleanupDone))
	require.NoError(t, err)
	require.NoError(t, lk.Release())

	lockPath := filepath.Join(dataDir, "locks", "session-"+sess.ID+".lock")

	// Join the exact cleanup completion, including the deferred close inside
	// clearHolderMetadata and both sidecar removals, before changing mtime.
	select {
	case <-cleanupDone:
	case <-time.After(2 * time.Second):
		t.Fatal("Release metadata cleanup did not complete before back-dating")
	}
