// End-to-end grep tool tests: identical queries run through BOTH search
// implementations (regex fallback and ripgrep), covering ignore-file
...
func TestGrepWithIgnoreFiles(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(tempDir, ".rushignore"), []byte(crushignoreContent), 0o644))
