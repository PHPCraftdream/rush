...
// Findings-driven tests for the add and replace paths: store-generation changes during preparation, transaction rollback, and the guards that decide whether a prepared client is ever published. Split out of lifecycle_findings_test.go when the 1000-line file limit landed.
