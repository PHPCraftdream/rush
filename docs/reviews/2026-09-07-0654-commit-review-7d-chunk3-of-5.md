...
### C3-5 (P3, ЗАКРЫТО 2026-09-09): test-mirror `r6_1DangerousToolNames` разошёлся с продовым списком

**ЗАКРЫТО** `063be9db` (2026-09-07): `agentic_fetch` добавлен в
`r6_1DangerousToolNames`; зеркало снова совпадает с
`libraryEphemeralDisabledTools` (сверено 2026-09-09).
...
| C3-5 | P3 | `9c82c0c2` | `r6_1DangerousToolNames` (`sdk/r6_1_library_mode_diskless_test.go:52-57`) не получил `agentic_fetch`, хотя его комментарий `:49-51` заявляет зеркалирование `libraryEphemeralDisabledTools` (`sdk/library_mode.go:209`). Headline-тест не поймает удаление записи из продового списка | **ЗАКРЫТО** `063be9db` (сверено 2026-09-09) |
...
**Подождут:** C3-2, C3-4 — два однострочных фикса (`trap`/`rm`, проверка
`[ -n "$pkgs" ]`); C3-5 закрыт в `063be9db` (сверка 2026-09-09). C3-3 стоит закрыть, сняв фиксированные таймауты с
